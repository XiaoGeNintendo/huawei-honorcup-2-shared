	...
	for(int x=0;x<8;x++)
	{
		for(int y=0;y<8;y++)
		{
			for(int k=0;k<4;k++)
			{
				...
				int ny=y+dy[k];
				if(out(nx,ny)) continue;
				newadj[choose[x*8+y]][choose[nx*8+ny]][k]+=(T%5);
			}
		}
	}
}
...
//		Solve();
	...
	for(int i=0;i<64;i++)
	{
		for(int j=0;j<64;j++)
		{
			for(int d=0;d<4;d++)
			{
				double t=((double)newadj[i][j][d]-(double)WeightSum*0.25)/WeightSum*15;
				cout<<sigmoid(t)<<' ';
			}
			cout<<endl;
		}
