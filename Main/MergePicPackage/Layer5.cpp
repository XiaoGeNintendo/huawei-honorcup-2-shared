	...
	for(int x=0;x<8;x++)
	{
		for(int y=0;y<8;y++)
		{
			for(int k=0;k<2;k++)
			{
				...
				int ny=y+dy[k];
				if(out(nx,ny)) continue;
				newadj[choose[x*8+y]][choose[nx*8+ny]][k]+=(T%16)*(T%16);
			}
		}
	}
}
...
//		Solve();
	...
	for(int i=0;i<64;i++)
	{
		for(int j=0;j<64;j++)
		{
			...
			{
//				assert(newadj[i][j][d])
				newadj[i][j][d]=newadj[j][i][d^2]=max(newadj[j][i][d^2],newadj[i][j][d]);
			}
		}
	}
	for(int i=0;i<64;i++)
	{
		for(int j=0;j<64;j++)
		{
			for(int d=0;d<4;d++)
			{
				double t=(double)newadj[i][j][d]/WeightSum;
				if(t>=0.06) cout<<1<<' ';else cout<<0<<' ';
//				double t=((double)newadj[i][j][d]-(double)WeightSum*0.3)/WeightSum*15;
//				cout<<sigmoid(t)<<' ';
			}
			cout<<endl;
		}
